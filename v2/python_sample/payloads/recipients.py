...
type_fields = {"last_name": str, "first_name": str, "email": str, "role_id": int, "recipient_id": int}
...
class EnvelopeRecipients:
    ...
    def __init__(self, *args, **kwargs):
        mandatory_fields = ["first_name", "email", "recipient_id"]
        for key, val in kwargs.items():
            setattr(self, key, val)
    ...
    def __str__(self):
        return json.dumps({"last_name": getattr(self, "last_name", None), "first_name": self.first_name,
                           "email": self.email, "recipient_id": self.recipient_id})
